          ...
          sdk.healthCheck.runHealthScript(
            [
              ...
              [
                ...
                'peers=$(printf "%s" "$json_lc" | sed -n \'s/.*"peercount"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\\1/p\')',
                'tip=$(printf "%s" "$json_lc" | sed -n \'s/.*"currenttipblockheight"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\\1/p\')',
                'sources=$(printf "%s" "$json_lc" | sed -n \'s/.*"localminingsourcecount"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\\1/p\')',
                'hashrate=$(printf "%s" "$json" | sed -n \'s/.*"localMiningHashrateDisplay"[[:space:]]*:[[:space:]]*"\\([^"\\]*\\)".*/\\1/p\')',
                'rpc_section=$(printf "%s" "$json_lc" | sed -n \'s/.*"rpc"[[:space:]]*:[[:space:]]*{\\([^}]*\\)}.*/\\1/p\')',
                'rpc=$(printf "%s" "$rpc_section" | sed -n \'s/.*"reachable"[[:space:]]*:[[:space:]]*\\([^,}]*\\).*/\\1/p\')',
                'synced=$(printf "%s" "$rpc_section" | sed -n \'s/.*"synced"[[:space:]]*:[[:space:]]*\\([^,}]*\\).*/\\1/p\')',
          ...
          sdk.healthCheck.runHealthScript(
            [
              ...
              [
                'json=$(curl -fsS --max-time 5 http://127.0.0.1:5000/api/network/summary | tr -d "\\r\\n")',
                'json_lc=$(printf "%s" "$json" | tr "[:upper:]" "[:lower:]")',
                'pulses=$(printf "%s" "$json_lc" | sed -n \'s/.*"localpulseacceptedcount"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\\1/p\')',
                ...
                'healthy=$(printf "%s" "$json_lc" | sed -n \'s/.*"outboundrelayhealthy"[[:space:]]*:[[:space:]]*\\([^,}]*\\).*/\\1/p\')',
                'enabled=$(printf "%s" "$json_lc" | sed -n \'s/.*"pulseproofsenabled"[[:space:]]*:[[:space:]]*\\([^,}]*\\).*/\\1/p\')',
