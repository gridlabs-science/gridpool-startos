          ...
          sdk.healthCheck.runHealthScript(
            [
              ...
              [
                ...
                'sources=$(printf "%s" "$json_lc" | sed -nE \'s/.*"localminingsourcecount"[[:space:]]*:[[:space:]]*([0-9]+).*/\\1/p\')',
                'hashrate=$(printf "%s" "$json" | sed -nE \'s/.*"localMiningHashrateDisplay"[[:space:]]*:[[:space:]]*"([^"]*)".*/\\1/p\')',
                'rpc=$(printf "%s" "$json_lc" | sed -nE \'s/.*"rpc"[[:space:]]*:[[:space:]]*[{][^}]*"reachable"[[:space:]]*:[[:space:]]*([^,}]*).*/\\1/p\')',
                'synced=$(printf "%s" "$json_lc" | sed -nE \'s/.*"rpc"[[:space:]]*:[[:space:]]*[{][^}]*"synced"[[:space:]]*:[[:space:]]*([^,}]*).*/\\1/p\')',
          ...
          sdk.healthCheck.runHealthScript(
            [
              ...
              [
                ...
                'lastRelay=$(printf "%s" "$json" | sed -nE \'s/.*"lastSuccessfulOutboundRelayUtc"[[:space:]]*:[[:space:]]*"([^"]*)".*/\\1/p\')',
                'healthy=$(printf "%s" "$json_lc" | sed -nE \'s/.*"outboundrelayhealthy"[[:space:]]*:[[:space:]]*([^,}]*).*/\\1/p\')',
                'enabled=$(printf "%s" "$json_lc" | sed -nE \'s/.*"pulseproofsenabled"[[:space:]]*:[[:space:]]*([^,}]*).*/\\1/p\')',
